...
const Pricing: React.FC = () => {
  ...
  useEffect(() => {
    const loadPlans = async () => {
      ...
      if (plans && userData) {
        const selectedPlans = plans.filter((plan: Plan) => (userData.age) < plan.age);
        ...
        <div className={styles.container}>
          ...
          <div className={styles.content}>
            <div className={styles.text}>
              <h1 className={styles.text__title}>{userData.name} ¿Para quién deseas cotizar?</h1>
              <p className={styles.text__description}>Selecciona la opción que se ajuste más a tus necesidades.</p>
            </div>
                      ...
                      <div className={styles.plan}>
                        <div className={styles.plan__header}>
                          <div className={styles.plan__header__title}>
                            <h3 className={styles.plan__header__title__text}>{plan.name}</h3>
                            <img src={plan.icon} alt={plan.name} />
                          </div>
                          <div className={styles.plan__header__cost}>
                            <p className={styles.plan__header__cost__label}>Costo del plan</p>
                            {isForSomeone ? (
                              <p className={styles.plan__header__cost__past}>${plan.price} antes</p>
                            ) : <></>}
                            <p className={styles.plan__header__cost__value}>${isForSomeone ? plan.discountPrice : plan.price} al mes</p>
                          </div>
                        </div>
